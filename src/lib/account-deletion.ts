...
export async function beforeAccountDelete(user: { id: string; email: string; isAdmin?: boolean }) {
  // 데모 계정은 비밀번호가 화면 코드에 그대로 있어서 누구나 로그인할 수 있어요 —
  // 방문자 한 명이 지워버리면 데모 버튼이 영영 망가짐.
  if (user.email === DEMO_EMAIL) {
    throw new APIError("FORBIDDEN", { message: "데모 계정은 탈퇴할 수 없어요." });
  }
  // 관리자는 실수로 지우면 관리 화면 접근이 사라져서 앱에서는 막고, 필요하면 DB에서
  // 직접 처리합니다.
  if (user.isAdmin) {
    throw new APIError("FORBIDDEN", { message: "관리자 계정은 앱에서 탈퇴할 수 없어요." });
  }
